...
export type EventCategory = "battle" | "territorial" | "political" | "military" | "humanitarian" | "milestone";
...
export const EVENT_CATEGORIES: EventCategoryInfo[] = [
  { id: "battle", label: "Battles", icon: <TbSword className="h-3.5 w-3.5" />, color: "text-destruction" },
  { id: "territorial", label: "Territory", icon: <TbFlag className="h-3.5 w-3.5" />, color: "text-damage" },
  { id: "political", label: "Political", icon: <TbUsers className="h-3.5 w-3.5" />, color: "text-ua-yellow" },
  { id: "military", label: "Military", icon: <TbShieldCheckered className="h-3.5 w-3.5" />, color: "text-capture" },
  { id: "humanitarian", label: "Humanitarian", icon: <TbBomb className="h-3.5 w-3.5" />, color: "text-destruction" },
  { id: "milestone", label: "Milestones", icon: <TbCalendarEvent className="h-3.5 w-3.5" />, color: "text-muted-foreground" },
];
...
export function getEventCategory(label: string): EventCategory {
  ...
  if (lower.includes("offensive") || lower.includes("battle") || lower.includes("siege"))
    return "battle";
  if (lower.includes("falls") || lower.includes("captured") || lower.includes("occupied") || lower.includes("recaptured") || lower.includes("liberated"))
    return "territorial";
  if (lower.includes("summit") || lower.includes("election") || lower.includes("inaugurated") || lower.includes("ceasefire"))
    return "political";
  if (lower.includes("massacre") || lower.includes("strikes") || lower.includes("dam") || lower.includes("sinks") || lower.includes("moskva") || lower.includes("bridge"))
    return "humanitarian";
  if (lower.includes("sanctions") || lower.includes("pledged") || lower.includes("spider") || lower.includes("dprk") || lower.includes("deployed"))
    return "military";
  return "milestone";
}
